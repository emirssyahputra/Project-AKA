...
(function() {
 ...
 /* ================================================
  ...
  const options = {
    rootMargin: '0px',
    threshold: 0.5 // Ketika elemen terlihat setidaknya 50%
  };

  // Fungsi untuk membuat counter berhitung saat elemen terlihat
  function startCounting(entries, observer) {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        const target = entry.target;
        const countTarget = +target.getAttribute('data-target');
        const countSpeed = 200; // Kecepatan hitungan dalam milidetik
        const increment = countTarget / countSpeed;
        ...
        observer.unobserve(target); // Berhenti mengamati elemen setelah counter dimulai
      }
    });
  }
