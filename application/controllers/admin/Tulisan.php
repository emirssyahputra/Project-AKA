...
class Tulisan extends CI_Controller{
	...
	function acc($pinjam_id){
		$status = "Diterima";
		$this->m_tulisan->edit_status($pinjam_id, $status);
		echo $this->session->set_flashdata('msg','success');
		redirect('admin/lihatpinjam');
	}
	function dec($pinjam_id) {
	...
	function hapus_lihatpinjam(){
		$kode=$this->input->post('kode');
		$this->m_tulisan->hapus_peminjaman($kode);
    ...
    $url = $protocol . $domain . '/artikel/' . $kode;

    // Generate the QR code as a string
    ob_start();
    \QRcode::png($url, null, QR_ECLEVEL_L, 4, 1);
    $qrCodeString = ob_get_clean();

    // Set the appropriate HTTP headers for image display
