...
class M_tulisan extends CI_Model{
	...
	function hapus_peminjaman($kode){
		$hsl=$this->db->query("delete from tbl_pinjam where pinjam_id='$kode'");
		return $hsl;
	}
	function get_peminjaman_by_kode($kode){
		$hsl=$this->db->query("SELECT tbl_pinjam.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_pinjam where pinjam_id='$kode'");
		return $hsl;
	}
	function get_all_peminjaman(){
		$hsl=$this->db->query("SELECT tbl_pinjam.*,DATE_FORMAT(pinjam_waktu,'%d/%m/%Y') AS tanggal FROM tbl_pinjam ORDER BY pinjam_id DESC");
		return $hsl;
	}
	function simpan_peminjaman($user_nama,$waktu,$deskripsi){
		$hsl=$this->db->query("insert into tbl_pinjam(pinjam_nama,pinjam_waktu,pinjam_deskripsi) values ('$user_nama','$waktu','$deskripsi')");
		return $hsl;
	}
	function edit_status($pinjam_id, $status) {
		$hsl = $this->db->query("UPDATE tbl_pinjam SET pinjam_status='$status' WHERE pinjam_id='$pinjam_id'");
		return $hsl;
	}
	//Front-End
	...
	function get_berita_home(){
		$hsl=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_tulisan ORDER BY tulisan_id DESC limit 9");
		return $hsl;
	}
  	function get_berita_blog(){
		$hsl=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_tulisan ORDER BY tulisan_id ");
		return $hsl;
	}
